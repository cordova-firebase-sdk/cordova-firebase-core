...
export class BaseArrayClass extends BaseClass {
   ...
   */
  public _mapSeries(iteratee: (item: any, idx: number, next: (result: any) => void) => void): Promise<any[]> {
    if (this.array.length === 0) {
      return Promise.resolve([]);
    }
    const results: any[] = [];
    const goal: number = this.array.length;
    const looper = (idx: number, resolve: (results: any[]) => void) => {
   ...
   * @returns a promise with transformed values
   */
  public _mapAsync(iteratee: (item: any, idx: number, next: (result: any) => void) => void): Promise<any[]> {
    if (this.array.length === 0) {
      return Promise.resolve([]);
    }
    const results: any[] = [];
    ...
    const goal: number = this.array.length;
    const looper = (item: any, idx: number, resolve: (results: any[]) => void) => {
      iteratee(this.array[idx], idx, (result: any) => {
        results[idx] = result;
        currentCnt++;
        if (currentCnt === goal) {
   ...
   */
  public _map(iteratee: (item: any) => any): any[] {
   ...
   */
  public _filterAsync(iteratee: (item: any, idx: number, next: (result: boolean) => void) => void): Promise<any[]> {
    if (this.array.length === 0) {
      return Promise.resolve([]);
    }
    const results: any[] = [];
   ...
   */
  public _filter(iteratee: (item: any, idx: number) => boolean): any[] {
    if (this.array.length === 0) {
      return [];
    }
    const results: any[] = this.array.filter(iteratee);
    return results;
  }
