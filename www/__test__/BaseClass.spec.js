...
describe("BaseClass test", () => {
    describe("_set()", () => {
        it("'instance.hello' should be 'world'", () => {
            const instance = new BaseClass_1.BaseClass();
    ...
    describe("_bindTo()", () => {
        it("'instanceB.anotherHello' should be the same as 'instanceA.hello'", () => {
            const instanceA = new BaseClass_1.BaseClass();
            const instanceB = new BaseClass_1.BaseClass();
    ...
    describe("_one()", () => {
        it("'instance._one()' should receive 'hello_changed' event.", (done) => {
            const instance = new BaseClass_1.BaseClass();
            instance._set("hello", "world");
            instance._one("hello_changed", (prevValue, newValue) => {
    ...
    describe("_on()", () => {
        ...
        it("'instance._on(one, two, three)' should receive `1, 2, 3`.", (done) => {
            (new Promise((resolve, reject) => {
                const instance = new BaseClass_1.BaseClass();
                const timer = setTimeout(reject, 10); // just in case
                instance._on("myEvent", (one, two, three) => {
                    expect(one).toBe(1);
                    expect(two).toBe(2);
                    expect(three).toBe(3);
                    resolve();
    ...
    describe("_trigger()", () => {
        it("'instance._trigger()' should fire a 'myEvent' event.", (done) => {
            (new Promise((resolve, reject) => {
                const instance = new BaseClass_1.BaseClass();
                const timer = setTimeout(reject, 10); // just in case
                instance._on("myEvent", (...parameters) => {
                    clearTimeout(timer);
                    resolve(parameters);
                });
